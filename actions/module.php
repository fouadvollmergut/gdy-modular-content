 ...
 *
 * @param mixed $redirect_url Redirect url value.
 */
...
/**
 * Handles the GDYMC activate module action action request.
 */
function gdymc_activate_module_action()
{
    $redirect_url = isset($_GET["redirect"])
        ? urldecode($_GET["redirect"])
        : admin_url("themes.php?page=gdymc-plugin-settings&tab=modules");

    if (
        !isset($_GET["gdymc_module_type"]) ||
        empty($_GET["gdymc_module_type"])
    ) {
...
add_action(
    "admin_post_gdymc_deactivate_module",
    "gdymc_deactivate_module_action"
);

/**
 * Handles the GDYMC deactivate module action action request.
 */
 ...
 * @param mixed $module_type Module type value.
 *
 * @param mixed $new_name New name value.
 *
 * @param mixed $redirect_url Redirect url value.
 */
...
/**
 * Handles the GDYMC rename module action action request.
 */
function gdymc_rename_module_action()
{
    $redirect_url = isset($_GET["redirect"])
        ? urldecode($_GET["redirect"])
        : admin_url("themes.php?page=gdymc-plugin-settings");

    if (
        !isset($_POST["gdymc_module_type"]) ||
        empty($_POST["gdymc_module_type"]) ||
        !isset($_POST["gdymc_module_name"]) ||
        empty($_POST["gdymc_module_name"])
    ) {
        return wp_redirect($redirect_url . "&status=error");
        exit();
    }
...
/**
 * Handles GDYMC save module behavior.
 */
function gdymc_save_module()
{
    if (
        ...
        return wp_redirect(
            admin_url(
                "themes.php?page=gdymc-plugin-settings&tab=modules&status=error"
            )
        );
        exit();
    }
    ...
    if (!$module_found) {
        return wp_redirect(
            admin_url(
                "themes.php?page=gdymc-plugin-settings&tab=modules&status=not-found"
            )
        );
        exit();
    }
    ...
    wp_redirect(
        admin_url(
            "themes.php?page=gdymc-plugin-settings&gdymc_module_type=" .
                urlencode($module_type) .
                "&status=saved"
        )
    );
    exit();
