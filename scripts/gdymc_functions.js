...
class ButtonGroup {
  /**
   * Handles GDYMC constructor behavior.
   *
   * @param {*} buttonGroupContainer Button group container value.
   */
  constructor(buttonGroupContainer) {
    this.buttonGroupContainer = buttonGroupContainer;
    this.buttonGroup = buttonGroupContainer.querySelector(
      ".gdymc_button-group",
    );
  ...
  /**
   * Initializes GDYMC init behavior.
   */
  init() {
    this.buttonGroupContainer
      .querySelectorAll(".gdymc_button_addbutton")
      .forEach((button) => {
        this.addButtonListener(button);
      });

    this.buttonGroupContainer
      .querySelectorAll(".gdymc_button_removebutton")
      .forEach((button) => {
        this.removeButtonListener(button);
      });

    this.buttonGroupContainer
      .querySelectorAll(".gdymc_button_editbutton")
      .forEach((button) => {
  ...
  /**
   * Removes GDYMC remove button listener data or controls.
   *
   * @param {*} button Button value.
   */
  ...
  /**
   * Handles GDYMC edit button listener behavior.
   *
   * @param {*} button Button value.
   */
  editButtonListener(button) {
    button.addEventListener("click", (event) => {
      this.editButton(
        event.target.closest(".gdymc_button_container").querySelector("a"),
      );
    });
  }

  /**
   * Adds GDYMC add button data or controls.
   */
  addButton() {
    this.buttonGroup.insertAdjacentHTML(
      "beforeend",
      `
  ...
  /**
   * Removes GDYMC remove button data or controls.
   *
   * @param {*} button Button value.
   */
  removeButton(button) {
  ...
  /**
   * Handles GDYMC edit button behavior.
   *
   * @param {*} button Button value.
   */
  editButton(button) {
    const data = {
      ...
      target: JSON.stringify(
        button.getAttribute("target") === "_blank" ? "true" : "false",
      ),
      type: JSON.stringify(
        button.classList.contains("button-primary") ? "true" : "false",
      ),
    };
    ...
    gdymc.ajax(
      ...
      function (response) {
        ...
        jQuery("body").append(
          '<div class="gdymc_overlay_link gdymc_overlay_window gdymc_inside gdymc_tabs_container">' +
            response +
            "</div>",
        );
        ...
        document.querySelector("#gdymc_saveedit_button").addEventListener(
          "click",
          function () {
            this.contextButton.textContent = document.querySelector(
              "#gdymc_editbutton_text",
            ).value;
            this.contextButton.setAttribute(
              "aria-label",
              document.querySelector("#gdymc_editbutton_text").value,
            );
            this.contextButton.setAttribute(
              "href",
              document.querySelector("#gdymc_insertlink_input").value,
            );
            this.contextButton.setAttribute(
              "target",
              document.querySelector("#gdymc_editbutton_target").checked
                ? "_blank"
                : "_self",
            );
            this.contextButton.classList.toggle(
              "button-primary",
              document.querySelector("#gdymc_editbutton_type").checked,
            );
  ...
  /**
   * Returns GDYMC gather button json data.
   */
  gatherButtonJSON() {
...
class OptionSortable {
   ...
   * Handles GDYMC constructor behavior.
   *
   * @param {*} sortableContainer Sortable container value.
   */
  constructor(sortableContainer) {
    this.sortableContainer = sortableContainer;
    this.sortableList = sortableContainer.querySelector(
      "ul.gdymc_option_sortable_list",
    );
    this.addButton = sortableContainer.querySelector(
      "button.gdymc_option_sortable_add",
    );
  }
  ...
  init() {
    ...
    this.sortableList
      .querySelectorAll(".gdymc_option_sortable_remove")
      .forEach((button) => {
  ...
  /**
   * Adds GDYMC add item data or controls.
   */
  addItem() {
    ...
    newItem
      .querySelector(".gdymc_option_sortable_remove")
  ...
  /**
   * Handles GDYMC update value behavior.
   */
  updateValue() {
    const items = Array.from(
      this.sortableList.querySelectorAll(".gdymc_option_sortable_item_value"),
    );
    ...
    const values = items
      .map((item) => item.dataset.uuid.trim())
      .filter((value) => value !== "");
    this.sortableContainer.querySelector(".gdymc_option_sortable").value =
      values.join(",");
...
jQuery(document).ready(function () {
  ...
  document
    .querySelectorAll(".gdymc_button-group_container")
    .forEach((buttonGroupContainer) => {
  ...
  jQuery(document.body).on("click", ".gdymc_tabs_button", function () {
    ...
    tabContainer
      .find('.gdymc_tabs_content[data-tab="' + tabTarget + '"]')
      .addClass("gdymc_active");
  });
  ...
  function gdymc_formatbuttons_resize() {
    ...
    if (
      space < 30 &&
      jQuery("#gdymc_formatbuttons button:visible:last").length > 0
    ) {
      ...
      space > 80 &&
      jQuery("#gdymc_formatbuttons button:hidden:first").length > 0
    ) {
    ...
    if (number > 0) {
      jQuery("#gdymc_adminbar > .gdymc_left").append(
        '<button id="gdymc_show_format_buttons" data-gdymc-tip="' +
          gdymc.lang("showallformattingoptions") +
          '"><span class="dashicons dashicons-editor-insertmore"></span><span id="gdymc_show_format_buttons_number">' +
          number +
          "</span></button>",
      );
    }
  ...
  jQuery(document).on(
    ...
    function (event) {
      jQuery("#gdymc_overlay_shadow").show();
      var holder = jQuery(
        '<div id="gdymc_show_format_buttons_window" class="gdymc_overlay_images gdymc_overlay_window gdymc_inside"></div>',
      ).appendTo("body");
    ...
    jQuery("#gdymc_dropzone_overlay").dropzone({
      ...
      init: function () {
        ...
        this.on("addedfile", function (file, xhr, formData) {
          ...
          jQuery("#gdymc_dropzone_uploader_container").removeClass(
            "gdymc_dz_finished",
          );

          var extension = file.name.split(".").pop();
          jQuery(file.previewElement).addClass("dz-filetype-" + extension);
          jQuery(file.previewElement).prepend(
            '<div class="dz-filetype">' + extension + "</div>",
          );
          ...
          jQuery("#gdymc_dropzone_previewContainer").removeClass(
            "gdymc_dz_finished",
          );
        });
        ...
        this.on("queuecomplete", function (file) {
          ...
          jQuery("#gdymc_dropzone_uploader_container").addClass(
            "gdymc_dz_finished",
          );
          /**
           * Sets GDYMC set timeout data.
        ...
        this.on("success", function (file, response) {
          jQuery(file.previewElement).wrap(
            '<a href="' + response + '" target="_blank"></a>',
          );
        });
  ...
  jQuery(document.body).on(
    "click",
    "#gdymc_dropzone_preview .dz-error",
    function () {
  ...
  /**
   * Handles GDYMC textmodal do behavior.
   *
  ...
  function gdymc_TextmodalDo(object) {
    ...
    gdymc_Textmodal = jQuery(
      "<div>" +
        characters +
        " / " +
        length +
        '<span class="gdymc_swapcontent" data-gdymc-tip="' +
        gdymc.lang("swap-content") +
        '"></span></div>',
    ).appendTo("body");
  ...
  /**
   * Handles GDYMC imagemodal do behavior.
   *
   * @param {*} object Object value.
   */
  function gdymc_imagemodalDo(object) {
    ...
    gdymc_imagemodal = jQuery("<div>" + iw + " x " + ih + "</div>").appendTo(
      "body",
    );
  ...
  jQuery(".gdymc_moduleoptionsbutton").click(function (e) {
    var moduleOverlay = jQuery(this)
      .parents(".gdymc_module")
      .find(".gdymc_overlay_module");
  ...
  jQuery("body").on(
    "click",
    "#gdymc_overlay_shadow, .gdymc_overlay_close, .gdymc_overlay_close_trigger",
    function () {
  ...
  jQuery("body").on("click", ".gdymc_insertlink_source-use", function () {
    ...
    jQuery(this)
      .parents(".gdymc_tabs_container")
      .find(".gdymc_tabs_button")
      .first()
      .trigger("click");
  });
  ...
  jQuery(".gdymc_image, .gdymc_gallery_container").click(function () {
    if (jQuery(document.body).hasClass("gdymc_edit")) {
      ...
      gdymc.ajax("gdymc_action_imageoverlay", data, function (response) {
        ...
        jQuery("#gdymc_overlay_shadow").show();
        jQuery("body").append(
          '<div class="gdymc_overlay_images gdymc_overlay_window gdymc_inside gdymc_tabs_container">' +
            response +
            "</div>",
        );
            ...
            .sortable({
              ...
              update: function (event, ui) {
                ...
                jQuery.each(jQuery(".gdymc_image_selection li"), function () {
                  sortedIDs.push(
                    jQuery.parseJSON(jQuery(this).attr("data-image")),
                  );
                });
  ...
  function gdymc_get_selected_images() {
    var ci = jQuery.parseJSON(
      jQuery("#gdymc_imagelist_holder").attr("data-ci"),
    );
  ...
  /**
   * Handles GDYMC set selected images behavior.
   *
   * @param {*} selectedImages Selected images value.
   */
  function gdymc_set_selected_images(selectedImages) {
    ...
    if (jQuery.isArray(selectedImages) && selectedImages.length > 0) {
      if (selectedImages.length > 1) {
        var text = gdymc
          .lang("imageinsert-plural")
          .replace("%s", selectedImages.length);
        button.html(text);
      } else {
        button.html(gdymc.lang("imageinsert-singular"));
      }

      jQuery("#gdymc_imagelist_holder").attr(
        "data-ci",
  ...
  /**
   * Handles GDYMC refresh image selection behavior.
   */
  function gdymc_refresh_image_selection() {
    ...
    jQuery.each(selectedImages, function (index, value) {
      jQuery('.gdymc_imagethumb[data-id="' + value[0] + '"]').addClass(
        "gdymc_selected",
      );
    });
  ...
  /**
   * Handles GDYMC refresh image thumbs behavior.
   */
  function gdymc_refresh_image_thumbs() {
    ...
    jQuery.each(selectedImages, function (index, value) {
      var thumbElement = jQuery(
        '<li class="gdymc_imagethumb" data-image=\'' +
          JSON.stringify(value) +
          "' data-id=\"" +
          value[0] +
          '"></li>',
      ).append(
        jQuery(".gdymc_mediathumb_" + value[0])
          .first()
          .clone(),
      );
  ...
  /**
   * Handles GDYMC remove from selection behavior.
   *
   * @param {*} imageID Image id value.
   */
  ...
  /**
   * Handles GDYMC add to selection behavior.
   *
   * @param {*} imageID Image id value.
   *
   * @param {*} isVideo Is video value.
   */
  function gdymc_add_to_selection(imageID, isVideo) {
    imageID = parseInt(imageID);

    // Remove selected if only one is allowed
    if (jQuery("#gdymc_imagelist_holder").attr("data-multiple") == "false")
      gdymc_set_selected_images("");
  ...
  /**
   * Handles GDYMC imageinfo localsettings visibility behavior.
   */
  function gdymc_imageinfo_localsettings_visibility() {
    var selectedImages = gdymc_get_selected_images();
    var editing = jQuery("#gdymc_overlay_content_imageinfoinner").attr(
      "data-id",
    );
    var isVideo =
      jQuery("#gdymc_overlay_content_imageinfoinner").attr("data-mime") ==
      "video";
    ...
    jQuery.each(selectedImages, function (key, value) {
      if (value[0] == editing) {
        ...
        if (isVideo) {
          var videoOptions =
            value[3] && typeof value[3] === "object"
              ? value[3]
              : { controls: false, autoplay: true, muted: true };

          jQuery("#gdymc_videoinfo_controls").prop(
            "checked",
            !!videoOptions.controls,
          );
          jQuery("#gdymc_videoinfo_autoplay").prop(
            "checked",
            !!videoOptions.autoplay,
          );
          jQuery("#gdymc_videoinfo_muted").prop(
            "checked",
            !!videoOptions.muted,
          );
        } else {
  ...
  jQuery(document.body).on(
    "click",
    "#gdymc_imagelist_holder .gdymc_imagethumb",
    function () {
      ...
      if (!current.hasClass("gdymc_selected")) {
        if (currentImageType == "exact") {
          ...
          gdymc_add_to_selection(
            currentImageID,
            current.attr("data-mime") == "video",
          );
          ...
          var cropTarget = jQuery('<img src="' + currentImageGuid + '" />')
            .appendTo("#gdymc_cropholder")
            .attr("src", currentImageGuid);
          ...
          /**
           * Handles GDYMC gdymcappendcropbutton behavior.
           */
  ...
  jQuery(document.body).on(
    "change",
    ".gdymc_imageinfo_local_input",
    function () {
      var current = jQuery(this);
      current
        .parents("#gdymc_overlay_content_imageinfo_local")
        .addClass("gdymc_active");

      var id = parseInt(
        jQuery("#gdymc_overlay_content_imageinfoinner").attr("data-id"),
      );
      var isVideo =
        jQuery("#gdymc_overlay_content_imageinfoinner").attr("data-mime") ==
        "video";
  ...
  jQuery(document.body).on(
    "change",
    ".gdymc_imageinfo_global_input",
    function () {
      var current = jQuery(this);
      current
        .parents("#gdymc_overlay_content_imageinfo_global")
        .addClass("gdymc_active");

      var imageID = jQuery("#gdymc_overlay_content_imageinfoinner").attr(
        "data-id",
      );
  ...
  jQuery(document.body).on("click", "#gdymc_cropbutton", function () {
    ...
    gdymc.ajax("gdymc_action_cropimage", data, function (response) {
      if (response[0] != "{") {
        ...
        jQuery(
          '.gdymc_overlay_images .gdymc_tabs_button[data-mode="exact"]',
        ).click();
   ...
   *
   * @param {*} beforeplace Beforeplace value.
   */
  function refresh_image_list(responsemode, beforeplace) {
    ...
    var mode = jQuery(
      ".gdymc_overlay_images .gdymc_tabs_button.gdymc_active",
    ).attr("data-mode");
  ...
  jQuery(document.body).on(
    "click",
    ".gdymc_overlay_images .gdymc_tabs_button",
    function () {
      jQuery("#gdymc_imagelist_holder").attr("data-p", 1);
      refresh_image_list();
    },
  ...
  jQuery(document.body).on("click", "#gdymc_loadmore_images", function () {
    var currentPage = parseInt(
      jQuery("#gdymc_imagelist_holder").attr("data-p"),
    );
  ...
  /**
   * Handles GDYMC gdymctable getrows behavior.
   *
  ...
  /**
   * Handles GDYMC gdymctable getcols behavior.
   *
   * @param {*} table Table value.
   */
  ...
  /**
   * Handles GDYMC gdymctable disable behavior.
   */
  ...
  /**
   * Handles GDYMC gdymctable enable behavior.
   */
  function gdymctable_enable() {
    jQuery(".gdymc_table td").each(function () {
      jQuery(this)
        .attr("contenteditable", "true")
        .addClass("gdymc_editable mousetrap");
    });
  }

  /**
   * Handles GDYMC gdymctable getjson behavior.
   *
   * @param {*} table Table value.
   */
  ...
  jQuery(document.body).on("click", ".gdymc_table_addrow", function () {
    ...
    tableObject = jQuery(this)
      .parents(".gdymc_table_container")
      .find(".gdymc_table");
  ...
  jQuery(document.body).on(
    "mouseenter focus",
    "[data-gdymc-tip]:not(.gdymc_active)",
    function () {
      ...
      jQuery(
        '<div id="gdymc_tooltip" class="gdymc_inside"><div id="gdymc_tooltip_arrow"></div>' +
          message +
          "</div>",
      ).appendTo("body");
      ...
      if (tooltipWidth + sourceLeft > windowWidth) {
        jQuery("#gdymc_tooltip")
          .addClass("gdymc_tooltip_right")
          .css({ top: sourceTop, right: sourceRight });
      } else {
        jQuery("#gdymc_tooltip")
          .addClass("gdymc_tooltip_left")
          .css({ top: sourceTop, left: sourceLeft });
      }
    },
  );

  jQuery(document.body).on(
    "mouseleave mousedown click blur",
    "[data-gdymc-tip]",
    function () {
      jQuery("#gdymc_tooltip").remove();
    },
  ...
  jQuery(document.body).on(
    "change",
    ".gdymc_change_single_moduletype",
    function () {
  ...
  jQuery(".gdymc_editable, .gdymc_option").bind(
  ...
  jQuery("#gdymc_modules_search").fastLiveFilter("#gdymc_modules_inner", {
    callback: function (total) {
      if (total == 0) {
        if (jQuery("#gdymc_modules_noresults").length == 0)
          jQuery("#gdymc_modules_inner").append(
            '<div id="gdymc_modules_noresults">' +
              gdymc.lang("nomodulesfound") +
              "</div>",
          );
      } else {
