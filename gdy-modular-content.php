...
define("GDYMC_PLUGIN_VERSION", "0.10.2");
...
/**
 * Handles GDYMC load textdomain behavior.
 */
...
/**
 * Handles GDYMC force locale behavior.
 *
...
$gdymc_module_folders = []; // Holds the list of module folders
$gdymc_module_types = []; // Holds the list of modules
$gdymc_modules = []; // Holds the list of modules
...
/**
 * Handles GDYMC plugin activation behavior.
 */
...
/**
 * Handles GDYMC plugin deactivation behavior.
 */
...
/**
 * Handles GDYMC plugin uninstall behavior.
 */
...
/**
 * Handles GDYMC preview setup behavior.
 */
function gdymc_preview_setup()
{
    ...
    if (is_user_logged_in()):
        if (
            isset($_GET["gdymc_hardpreview"]) and
            $_GET["gdymc_hardpreview"] == 0
        ):
            gdymc_remove_cookie("gdymc_hardpreview");
            gdymc_remove_cookie("gdymc_softpreview");
        elseif (
            isset($_COOKIE["gdymc_hardpreview"]) and
                $_COOKIE["gdymc_hardpreview"] == 1 and
                !is_admin() and
                $GLOBALS["pagenow"] != "wp-login.php" or
            isset($_GET["gdymc_hardpreview"]) and
                $_GET["gdymc_hardpreview"] == 1
        ):
...
/**
 * Handles GDYMC preview reset behavior.
 */
...
/**
 * Handles GDYMC disable wordpress adminbar behavior.
 */
...
/**
 * Handles GDYMC body class behavior.
 *
 * @param mixed $classes Classes value.
 */
function gdymc_body_class($classes = "")
{
    ...
    if (!gdymc_preview() and gdymc_logged()) {
        $classes[] = "gdymc_edit";
    }
    if (!gdymc_logged() and !gdymc_hardpreview()) {
        $classes[] = "gdymc_visitor";
    }
    if (gdymc_hardpreview()) {
        $classes[] = "gdymc_hardpreview gdymc_visitor";
    }
    if (gdymc_softpreview()) {
        $classes[] = "gdymc_softpreview";
    }
    if (gdymc_hardpreview()) {
...
/**
 * Handles GDYMC shutdown behavior.
 */
function gdymc_shutdown()
{
    ...
    if (is_array($gdymc_object_contents)) {
        update_metadata(
            ...
            json_encode($gdymc_object_contents)
        );
    }
}
...
add_action("wp_enqueue_scripts", function () {
    ...
    if (gdymc_logged()):
        ...
        wp_enqueue_style(
            "gdymc_style",
            plugins_url("/_styles/style.css", __FILE__),
            ["dashicons"],
        ...
        wp_enqueue_script(
            "log4javascript",
            plugins_url("/scripts/log4javascript.js", __FILE__),
            ["jquery"]
        );
        wp_enqueue_script(
            "rangy_core",
            plugins_url("/scripts/rangy_core.js", __FILE__),
            ["log4javascript"]
        );
        wp_enqueue_script(
            "rangy_selectionsaverestore",
            plugins_url("/scripts/rangy_selectionsaverestore.js", __FILE__),
            ["log4javascript", "rangy_core"]
        );
        wp_enqueue_script(
            "rangy_classapplier",
            plugins_url("/scripts/rangy_classapplier.js", __FILE__),
            ["log4javascript", "rangy_core"]
        );
        wp_enqueue_script(
            "kinetic",
            plugins_url("/scripts/kinetic.js", __FILE__),
            ["jquery"]
        );
        wp_enqueue_script(
            "mousetrap",
            plugins_url("/scripts/mousetrap.js", __FILE__),
            ["jquery"]
        );
        wp_enqueue_script(
            "dropzone",
            plugins_url("/scripts/dropzone.js", __FILE__),
            ["jquery"]
        );
        wp_enqueue_script(
            "fastlivefilter",
            plugins_url("/scripts/filter.js", __FILE__),
            ["jquery"]
        );
        wp_enqueue_script(
            "gdymc_core",
            plugins_url("/scripts/gdymc_core.js", __FILE__),
            ["jquery"],
            GDYMC_PLUGIN_VERSION
        );
        wp_enqueue_script(
            "gdymc_functions",
            plugins_url("/scripts/gdymc_functions.js", __FILE__),
            [
                "jquery",
                "jquery-ui-core",
                "jquery-ui-sortable",
        ...
        wp_localize_script("gdymc_core", "gdymc_dynamic_data", [
    ...
    if (gdymc_hardpreview()):
        ...
        wp_enqueue_style(
            "gdymc_hardpreview",
            plugins_url("/_styles/hardpreview.css", __FILE__),
            [],
            GDYMC_PLUGIN_VERSION
        );
        ...
        wp_enqueue_script(
            "gdymc_hardpreview",
            plugins_url("/scripts/gdymc_hardpreview.js", __FILE__),
            ["utils"],
            GDYMC_PLUGIN_VERSION
        );
        ...
        wp_localize_script("gdymc_hardpreview", "gdymc_dynamic_data", [
            "cookie_path" => COOKIEPATH,
            "cookie_domain" => COOKIE_DOMAIN,
        ]);
    endif;
