...
/**
 * Handles GDYMC module option option section behavior.
 *
...
{
    if (gdymc_logged()):
        echo '<div class="gdymc_error gdymc_inside"><span class="dashicons dashicons-info"></span>' .
            $errorMessage .
            "</div>";
    endif;
}
 ...
 *
 * @param mixed $moduleIDP Module idp value.
 *
 * @param mixed $objectIDP Object idp value.
 *
 * @param mixed $objectTypeP Object type p value.
 */
function optionExists(
    ...
    if (
        metadata_exists(
            $objectType,
            $objectID,
            "_gdymc_" . $moduleID . "_option_" . $optionName
        )
    ):
        return true;
    else:
 ...
 * @param mixed $optionName Option name value.
 *
 * @param mixed $optionValue Option value value.
 *
 * @param mixed $moduleIDP Module idp value.
 *
 * @param mixed $objectIDP Object idp value.
 ...
 * @param mixed $optionName Option name value.
 *
 * @param mixed $optionSettings Option settings value.
 *
...
function optionInput($optionName, $optionSettings, $moduleID)
{
    ...
    $attributeDefaults = [
        ...
        "class" =>
            "gdymc_option gdymc_option_" . strtolower($optionSettings["type"]),
        "placeholder" => $settings["placeholder"],
        "type" => $settings["type"],
        "value" => optionGet($optionName, $moduleID),
    ];

    $attributesArray = wp_parse_args(
        $settings["attributes"],
        $attributeDefaults
    );
    ...
    if ($settings["type"] != "hidden") {
        echo '<div class="gdymc_formpart gdymc_optioncontainer gdymc_optioncontainer_' .
    ...
    if (in_array($settings["type"], ["area", "textarea"])):
        echo "<textarea " .
            $attributes .
            ">" .
            $attributesArray["value"] .
            "</textarea>";
    elseif (in_array($settings["type"], ["select"])):
        ...
        echo "</select>";
    elseif (in_array($settings["type"], ["sortable"])):
        $sortableValue = !empty($attributesArray["value"])
            ? explode(",", $attributesArray["value"])
            : [];
        ...
        foreach ($sortableValue as $key => $value):
            ...
            echo '<span class="gdymc_option_sortable_item_handle dashicons dashicons-editor-code" aria-label="' .
                __("Sort Element", "gdy-modular-content") .
                '"></span>';
            echo '<div class="gdymc_option_sortable_item_value" data-uuid="' .
                esc_html($value) .
                '">' .
                substr(str_replace("-", "", esc_html($value)), 0, 8) .
                "</div>";
            echo '<button class="gdymc_button gdymc_option_sortable_remove" aria-label="' .
                __("Remove Element", "gdy-modular-content") .
                '"></button>';
            echo "</li>";
        endforeach;
        ...
        echo '<button class="gdymc_button gdymc_option_sortable_add">' .
            __("Add Element", "gdy-modular-content") .
            "</button>";
